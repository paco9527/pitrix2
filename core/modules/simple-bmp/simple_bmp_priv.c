...
bmp_data* do_read_bmp(char* path)
{
    ...
    ret = fread(temp_data.file_buf, temp_data.file_size, 1, bmp_file);
    if(0 == ret)// 返回写入temp_data.file_size的次数；0说明写入失败
    {
        free(temp_data.file_buf);
        return NULL;
    }
    temp_data.head = (bmp_head*)(temp_data.file_buf + CONST_FILE_HEAD_LEN); // 一个bmp文件从0偏移开始的组成顺序：通用文件头-bmp文件头-bmp图像数据
